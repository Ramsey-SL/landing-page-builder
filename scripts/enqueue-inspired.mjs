// Dev helper: reimagine a parent version in the style of a reference URL.
// Usage: node scripts/enqueue-inspired.mjs <parentVersionId> "<referenceUrl>"
...
if (!parentId || !referenceUrl) {
  console.error('Usage: node scripts/enqueue-inspired.mjs <parentVersionId> "<referenceUrl>"');
  process.exit(1);
}
...
console.log(`→ Reimagining parent ${parentId} in the style of\n  ${referenceUrl}`);
...
await post('jobs', { org_id: ORG_ID, version_id: child.id, kind: 'inspired', state: 'queued', progress: 0 });
